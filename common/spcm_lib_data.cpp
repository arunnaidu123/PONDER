...
bool bFillSB5Header (ST_SPCM_CARDINFO *pstCard, ST_SB5HEAD *pstHeader, char* pszName)
    {
    char szSource[25];
    int  i;
...
bool bSpcMCalcSignal (ST_SPCM_CARDINFO *pstCardInfo, void *pvData, uint32 dwLenInSamples, uint32 dwByteWidth, E_SPCM_SIGSHAPE eShape, uint32 dwLoops, uint32 dwGainP)
    ...
    uint32 dwBlock = dwLenInSamples / dwLoops;
    uint32 dwBlockHalf = dwBlock / 2;
    uint32 dwPosInBlock;
    ...
    for (i=0; i<dwLenInSamples; i++)
        ...
        switch (eShape)
            ...
            // sawtooth
            case eSawtooth:
                llValue = (int64) (llMinFS + dwPosInBlock * dSpan / dwBlock);
                break;
