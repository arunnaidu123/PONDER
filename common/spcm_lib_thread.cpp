...
spcm_lib_thread.cpp                            (c) Spectrum GmbH , 02/2006
...
// ***********************************************************************
    ...
    pstBufferData->llDataTransferred =  0;
    pstBufferData->llTSTransferred =    0;
    pstBufferData->llABATransferred =   0;
    ...
    while (!dwError)
        ...
        if (pstBufferData->bStartData)
            {
            spcm_dwGetParam_i32 (pstBufferData->pstCard->hDrv, SPC_DATA_AVAIL_USER_LEN,   (int32*) &pstBufferData->dwDataAvailBytes);
            spcm_dwGetParam_i32 (pstBufferData->pstCard->hDrv, SPC_DATA_AVAIL_USER_POS,   &lAvailPos);
            if ((lAvailPos + pstBufferData->dwDataAvailBytes) >= pstBufferData->dwDataBufLen)
                pstBufferData->dwDataAvailBytes = (uint32) (pstBufferData->dwDataBufLen - lAvailPos);
            pstBufferData->pvDataCurrentBuf = (void*) (((char*) pvDataBuffer) + lAvailPos);
            }
        ...
        if (pstBufferData->bStartTimestamp)
            {
            spcm_dwGetParam_i32       (pstBufferData->pstCard->hDrv, SPC_TS_AVAIL_USER_LEN,     (int32*) &pstBufferData->dwTSAvailBytes);
            spcm_dwGetParam_i32       (pstBufferData->pstCard->hDrv, SPC_TS_AVAIL_USER_POS,     &lAvailPos);
            if ((lAvailPos + pstBufferData->dwTSAvailBytes) >= pstBufferData->dwTSBufLen)
                pstBufferData->dwTSAvailBytes = pstBufferData->dwTSBufLen - lAvailPos;
            pstBufferData->pvTSCurrentBuf = (void*) (((char*) pvTSBuffer) + lAvailPos);
            }
        ...
        if (pstBufferData->bStartABA)
            {
            spcm_dwGetParam_i32       (pstBufferData->pstCard->hDrv, SPC_ABA_AVAIL_USER_LEN,     (int32*) &pstBufferData->dwABAAvailBytes);
            spcm_dwGetParam_i32       (pstBufferData->pstCard->hDrv, SPC_ABA_AVAIL_USER_POS,     &lAvailPos);
            if ((lAvailPos + pstBufferData->dwABAAvailBytes) >= pstBufferData->dwABABufLen)
                pstBufferData->dwABAAvailBytes = pstBufferData->dwABABufLen - lAvailPos;
            pstBufferData->pvABACurrentBuf = (void*) (((char*) pvABABuffer) + lAvailPos);
            }

        // if there is some new data we call our work function
        if (pstBufferData->dwDataAvailBytes || pstBufferData->dwTSAvailBytes || pstBufferData->dwABAAvailBytes)
            if (pfn_bWorkDo)
        ...
        if (pstBufferData->bStartData && pstBufferData->dwDataAvailBytes)
            spcm_dwSetParam_i32 (pstBufferData->pstCard->hDrv, SPC_DATA_AVAIL_CARD_LEN, pstBufferData->dwDataAvailBytes);

        if (pstBufferData->bStartTimestamp && pstBufferData->dwTSAvailBytes)
            spcm_dwSetParam_i32 (pstBufferData->pstCard->hDrv, SPC_TS_AVAIL_CARD_LEN, pstBufferData->dwTSAvailBytes);

        if (pstBufferData->bStartABA && pstBufferData->dwABAAvailBytes)
            spcm_dwSetParam_i32 (pstBufferData->pstCard->hDrv, SPC_ABA_AVAIL_CARD_LEN, pstBufferData->dwABAAvailBytes);
       

        // we continuously count the number of bytes that we have transferred so far
        pstBufferData->llDataTransferred +=   pstBufferData->dwDataAvailBytes;
        pstBufferData->llTSTransferred +=     pstBufferData->dwTSAvailBytes;
        pstBufferData->llABATransferred +=    pstBufferData->dwABAAvailBytes;
