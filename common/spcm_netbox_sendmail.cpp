...
void vNetboxMailSetSubject (const char* szSubject)
    ...
    if (g_szSubject != NULL)
        delete [] g_szSubject;
    g_szSubject = new char[strlen (szSubject)];
...
void vNetboxMailSetBody (const char* szBody)
    ...
    if (g_szBody != NULL)
        delete [] g_szBody;
    g_szBody = new char[strlen (szBody)];
...
void vNetboxMailAddAttachment (const char* szFilename)
    ...
    g_aszFilename[g_dwNextAttachmentIdx] = new char[strlen (szFilename)];
