...
struct ST_BUFFERDATA
    ...
    uint32              dwDataAvailBytes;       // current data length that is available
    int64               llDataTransferred;      // complete transferred amount of data

    void*               pvTSCurrentBuf;         // a pointer to the current timestamp buffer starting address
    uint32              dwTSAvailBytes;         // current timestamp length that is available
    int64               llTSTransferred;        // complete transferred amount of timestamp data

    void*               pvABACurrentBuf;        // a pointer to the current ABA buffe starting address
    uint32              dwABAAvailBytes;        // current ABA data length that is available
    int64               llABATransferred;       // complete transferred amount of ABA data
    };
